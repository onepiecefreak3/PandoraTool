...
[Generator]
public sealed class ScriptInstructionCalculatorGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(attributes, GenerateCalculator);
    }

    private void GenerateCalculator(SourceProductionContext productionContext, AttributeData attribute)
    {
        ...
        if (isManual)
        {
            productionContext.AddSource($"Instruction{instruction}Calculator.g.cs",
                $$"""
                  // <auto-generated />
                  ...
                  internal partial class Instruction{{instruction}}Calculator : ScriptInstructionCalculator
                  {
                      private static readonly Lazy<Instruction{{instruction}}Calculator> Lazy = new(() => new Instruction{{instruction}}Calculator());
                      
                      public static Instruction{{instruction}}Calculator Instance => Lazy.Value;
                  }
                  """);
        }
        else
        {
            ...
            productionContext.AddSource($"Instruction{instruction}Calculator.g.cs",
                ...
                internal class Instruction{{instruction}}Calculator : ScriptInstructionCalculator
                {
                    private static readonly Lazy<Instruction{{instruction}}Calculator> Lazy = new(() => new Instruction{{instruction}}Calculator());
    ...
    private static void ProcessArgumentExpression(StringBuilder sb, char mode, int count, int tab)
    {
        for (var j = 0; j < count; j++)
        {
            ...
            {
                case 'c':
                    sb.AppendLine($"{new string(' ', tab * 4)}CalculateValueData(instruction, argumentIndex++, ref length);");
                    break;

                case 'v':
                    sb.AppendLine($"{new string(' ', tab * 4)}CalculateVariableData(instruction, argumentIndex++, ref length);");
                    break;

                case 's':
                    sb.AppendLine($"{new string(' ', tab * 4)}CalculateStringData(instruction, argumentIndex++, ref length);");
                    break;

                case 'j':
                    sb.AppendLine($"{new string(' ', tab * 4)}CalculateJumpData(instruction, argumentIndex++, ref length);");
                    break;

                case 'i':
                    sb.AppendLine($"{new string(' ', tab * 4)}CalculateInt32(instruction, argumentIndex++, ref length, out _);");
                    break;
            }
        }
    }
